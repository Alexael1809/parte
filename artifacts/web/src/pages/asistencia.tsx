...
import { Save, ArrowLeft, Loader2, AlertCircle, CheckCircle, Lock, Users } from "lucide-react";
...
export default function AsistenciaPage() {
  ...
  useEffect(() => {
    if (!personas || initialized) return;
    const initial: Record<number, { estado: Estado; motivo: string }> = {};
    personas.forEach((p) => { initial[p.id] = { estado: "presente", motivo: "" }; });
    setRows(initial);
    setInitialized(true);
  }, [personas, initialized]);

  useEffect(() => {
    if (!asistenciasHoy || asistenciasHoy.length === 0 || !personas) return;
    const loaded: Record<number, { estado: Estado; motivo: string }> = {};
    personas.forEach((p) => { loaded[p.id] = { estado: "presente", motivo: "" }; });
    asistenciasHoy.forEach((a) => { loaded[a.personaId] = { estado: a.estado, motivo: a.motivo ?? "" }; });
    setRows(loaded);
    setInitialized(true);
  }, [asistenciasHoy]);
      ...
      <div className="p-4 md:p-6 max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <Button
            ...
            disabled={saveMutation.isPending || !personas?.length}
        ...
        {asistenciasHoy && asistenciasHoy.length > 0 && (
          <div className="mb-4 flex items-center gap-2 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
            ...
            <p className="text-blue-300 text-sm">Asistencia ya registrada hoy. Puedes modificarla.</p>
          </div>
        )}
        ...
        {[{ label: "Masculinos", list: masculinos }, { label: "Femeninas", list: femeninos }].map(({ label, list }) => (
          list.length > 0 && (
                ...
                {list.map((persona) => {
                  ...
                  return (
                    <div key={persona.id} className="bg-[#1B2B3D] border border-white/10 rounded-xl p-3">
                      ...
                      {REQUIRE_MOTIVO.includes(row.estado) && (
                        <input
                          type="text"
                          placeholder="Motivo..."
                          value={row.motivo}
        ...
        {personas && personas.length === 0 && !loadingPersonas && (
